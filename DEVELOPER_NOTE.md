...
- Edit source data in `content/*.json`.
- Generate site data with `node scripts/build_portfolio_data.js`.
- The website loads `generated/portfolio-data.js`.
- `scripts/portfolio-ui.js` renders that generated data in `index.html`; keep
  content changes in `content/*.json`.
...
scraping Scholar. It runs when a file in `content/` or
`scripts/build_portfolio_data.js` changes, and it can also be run manually.
...
For detailed examples, see `content/README.md`.
